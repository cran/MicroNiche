...
function(df, R, sampleInfo, envInfo, q = 1.65){
  ...
  for(k in 1:999){
    ...
    tmp.mat <- as.matrix(cbind(RA.proportions, as.matrix(envagg$x)))
    tmp.res <- vector()
    foo <- function(z,e) z - e
    for(q in 1:nrow(tmp.mat)){
      NB2 <- mapply(foo, max(tmp.mat[q,]), min(tmp.mat[q,]))
      tmp.res <- c(tmp.res,NB2)
    }
    Z$Null.Bn[k] <- 1 - 0.5*sum(tmp.res)
  }
  ...
  for(i in 1:nrow(Counts.ra)){
    ...
    colnames(envsubdf) <- c("SI", "EI")
    envagg <- aggregate(as.numeric(envsubdf[,2]), by=list(Category=envsubdf[,1]), FUN=sum)
    tmp.mat <- as.matrix(cbind(as.matrix(agg$x), as.matrix(envagg$x)))
    tmp.res <- vector()
    foo <- function(z,e) z - e
    for(q in 1:nrow(tmp.mat)){
      NB2 <- mapply(foo, max(tmp.mat[q,]), min(tmp.mat[q,]))
    ...
    res2 <- 1 - 0.5*sum(tmp.res)
    X <- mean(Z$Null.Bn)
    z <- (res2 - X)/(3/(sqrt(999)))
    pval <- 2*pnorm(-abs(z))
    tmp <- data.frame(PS = res2, P.val = pval)
